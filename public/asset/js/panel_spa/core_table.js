...
function load_table_active( btn_load ) {	
	...
	var URL_ENDPOINT = table_data_container.attr('data-api-endpoint');
	var FUNCTION_CLOSURE = table_data_container.attr('data-fungsi');
	...
	get_data( URL_ENDPOINT, {}, function( response ) {
		...
		if ( response != null ) {
			...
			load_table( FUNCTION_CLOSURE, data_load );
...
function load_table( FUNCTION_CLOSURE, data = [] ) {
	trace();
	...
	var banyak_data = $('.banyak_data');
	banyak_data.text( data.length );

	var nama_fungsi = "load_table_" + FUNCTION_CLOSURE;
...
//==================================== ENDLOAD TABLE FUNCTION CLOSURE, MODUL TEKNISI ====================================
